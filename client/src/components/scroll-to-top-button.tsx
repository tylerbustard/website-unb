...
export default function ScrollToTopButton({
  ...
  return (
    <button
      onClick={scrollToTop}
      className={`scroll-to-top-floating fixed z-40 rounded-full liquid-glass-pill transition-all duration-300 ease-in-out ${
    ...
    >
      <div className={`${compact ? "px-3 py-3" : "px-3 py-3 sm:px-4 sm:py-3"} flex items-center`}>
        {!compact && (
          <span className="mr-3 hidden text-sm font-medium text-white/90 sm:inline">
            Back to top
          </span>
        )}
